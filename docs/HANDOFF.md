...
## 2026-09-06 마감 시점의 상태 (이 절이 최신이다)

아래 본문은 08-31 기준이라 지금과 여러 곳이 다르다. 마감 시점의 상태는 이렇다.

| 항목 | 상태 |
|---|---|
| 평가용 API 서버 | `http://211.188.57.111:8000` 가동 중 |
| 자동 재시작 | Windows 작업 스케줄러 `DisclosureAgentAPI`. 무인 재부팅 복귀 28초 확인 |
| 검색 | 낱말(BM25) · 의미(CLOVA 임베딩) · 매칭표 세 갈래를 RRF 로 합침 |
| 정형 표 조회 | 재무 수치(`facts.py`) · 수시/거래소/지분 공시(`events.py`) |
| 생성 | HyperCLOVA X HCX-005 |
| 임베딩 | CLOVA v2 1,024차원. `chunk.embedding` 75.3% 채움 |
| 다중 기업 | 한 질의에서 최대 6곳. 이름이 없으면 그룹·업종·시장으로 추정 |
| 출력 검증 | 수치 · 출처 · 절 · 기업이 근거에 있는지. 걸리면 한 번 재생성하고 남으면 답변에 밝힘 |
| 자체 평가 | 28문항 두 차례. `docs/EVAL_REVIEW.md` |
| 제출물 | `제출물/` 폴더에 위치 안내 · API 서버 정보 · 기술 제안서 PDF |

실행 단계의 최신 목록은 `docs/PIPELINE.md` 맨 앞과 `docs/API.md` 에 있다.
그쪽이 코드와 맞춰 둔 문서다.
