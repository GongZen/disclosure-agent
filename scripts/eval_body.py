...
## 무엇을 비교하나
    ...
    --mode=ladder    낱말·의미·매칭표가 각각 얼마씩 보태는지 잰다
...
def main(mode: str = "weights", topk: int = 8,
    ...
    if mode == "weights":
        ...
        # 세 갈래가 각각 얼마씩 보태는지 한 조건에서 잰다.
        #
        # 지금까지 모드마다 조건이 달라 표를 나란히 못 뒀다. 제안서 4장
        # 도식이 낱말·의미·매칭표 셋을 그리는데 7장 측정에는 둘만 있어
        # 어긋난다는 지적이 있었다. 그 어긋남을 없애려는 것이다.
        #
        # 가중치는 (의미 벡터, 낱말 검색) 순이다. (0,1) 이면 낱말만,
        # (1,0) 이면 의미만, (1,1) 이면 둘을 순위로 합친다.
        print(f"\n── 사다리  (같은 후보 · 같은 질의)")
        print(f"{'설정':<26}{'1위':>7}{'3위내':>7}{f'{topk}위내':>8}")
        rungs = [
            ("낱말 검색만", (0.0, 1.0), False),
            ("의미 벡터만", (1.0, 0.0), False),
            ("낱말 + 의미 벡터", (1.0, 1.0), False),
            ("낱말 + 의미 + 매칭표", (1.0, 1.0), True),
        ]
        for name, w, uk in rungs:
            h1, h3, hk, _ = run(cp, cases, qvecs, w, base, topk, False, uk)
            print(f"{name:<26}{h1:>3}/{n}{h3:>4}/{n}{hk:>5}/{n}")
